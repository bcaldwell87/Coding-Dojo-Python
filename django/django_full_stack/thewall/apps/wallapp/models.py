...
class UserManager(models.Manager):
    def validate(self, request):
        ...
        if request.POST['pass'] != request.POST['pass_confirm']:
            messages.error(
                request, 'Password does not match password confirmation')
            errors = True
...
class Comment(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.DO_NOTHING, related_name='u_comments')
    message = models.ForeignKey(
        Message, on_delete=models.DO_NOTHING, related_name='m_comments')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    objects = UserManager()
